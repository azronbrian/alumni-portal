...
$name = $_SESSION['name'];
if (isset($name)) {
	$result=mysqli_query($conn,"select user_id,user_name,user_email,user_country,user_gender,user_reg ,user_course,user_branch,user_j_day,user_address,user_job from users order by user_id desc");
	$row = mysqli_num_rows($result);
	echo "<div class='container'>";
	echo "<h3> Welcome to Admin panel</h3>";
	echo "<h3>Total Registered users: $row</h3>";
	echo'<form class="search" action="search.php" method="POST">
	<input  type="text" name="search" placeholder="search">
	<button type="submit" name="submit-search">Search</button>
	</form>';
	echo '<form class="export"  action="export.php" method="POST">
